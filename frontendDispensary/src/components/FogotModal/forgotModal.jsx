...
const ForgotModal = (props) => {
 ...
 const [step, setStep] = useState(1);
    ...
    const sendOTPToMail = async () => {
        ...
        props.showLoader()
        await axios.post(`${import.meta.env.VITE_API_URL}/api/auth/send-otp`, { email: inputField.email }).then((response) => {
    ...
    const checkOtp = async () => {
        ...
        await axios.post(`${import.meta.env.VITE_API_URL}/api/auth/verify-otp`, { email: inputField.email, otp: inputField.otp }).then((response) => {
            setStep(3);
    ...
    const resetPassword = async () => {
        ...
        await axios.post(`${import.meta.env.VITE_API_URL}/api/auth/reset-password`, { email: inputField.email, newPassword: inputField.newPassword }).then((response) => {
            alert(response.data.message)
            props.closeModal()
        }).catch((err) => {
            console.log(err)
        ...
        <div className='forgot-password-modal'>
            <div className='signup-page-card'>
                ...
                <div className='form-input-fields'>
                    <input value={inputField.email} disabled={step !== 1} onChange={(e)=>handleOnChange(e,'email')} className='form-input' type='email' placeholder='Enter your mail id' />
                    ...
                    {step===3 && <input value={inputField.newPassword} onChange={(e)=>handleOnChange(e,'newPassword')} className='form-input' type='password' placeholder='Enter new password' />}
                </div>
