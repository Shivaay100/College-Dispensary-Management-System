...
const NearByHospital = (props) => {
    ...
    const fetchData = async () => {
        props.showLoader();
        await axios.get('${import.meta.env.VITE_API_URL}/api/hospital/get').then((resp) => {
    ...
    const handleDelete = async (id) => {
        props.showLoader();
        await axios.delete(`${import.meta.env.VITE_API_URL}/api/hospital/delete/${id}`, { withCredentials: true }).then((resp) => {
            filterOutData(id)
        }).catch(err => {
            toast.error(err?.response?.data?.error)
        }).finally(() => {
    ...
    return (
        <div className='admin-facility'>
            ...
            {modal && <Modal headers="Add Facility" handleClose={onOffModal} children={<NearByModal clickedItem={clickedItem} />} />}
            <ToastContainer />
        </div>
